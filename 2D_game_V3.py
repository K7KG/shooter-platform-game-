...
while running:
    ...
    if not game_over and not level_complete:
        ...
        for spike in spikes:
            ...
            # Adjust spike collision to only check the top part
            spike_collision_rect = (spike_rect[0], spike_rect[1] + spike_rect[3] - 5, spike_rect[2], 5)
            
            if check_collision(player_rect, spike_collision_rect) and player.invincibility == 0:
                player.health -= 20
                player.invincibility = 30
                if player.health <= 0:
                    game_over = True
